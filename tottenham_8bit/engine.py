...
class Mixer:
    ...
    def sfx(self, t, name):
        ...
        if name == "blip":
            ...
            ph = np.cumsum(f) / SR % 1.0
            a(t, ((2 * ph - 1) * 0.3 + _nrng.uniform(-1, 1, n) * 0.4) * np.exp(-tt / 0.3) * 0.35)
        elif name == "rewind":
            a(t, tone("sq", 200, 1.0, 0.08, duty=0.25, f1=1600))
            a(t, noise(1.0, 0.06, None, hold=3))
        elif name == "itemget":
            self.notes(t, "G4:0.12 A4:0.12 B4:0.12 C5:0.12 D5:0.12 E5:0.12 G5:0.7", vol=0.1, duty=0.25)
            self.notes(t, "E4:0.12 F4:0.12 G4:0.12 A4:0.12 B4:0.12 C5:0.12 E5:0.7", vol=0.06, duty=0.5)
            self.notes(t + 0.72, "C3:0.8", kind="tri", vol=0.2)
        elif name == "caw":
            for k in range(2):
                a(t + k * 0.28, tone("sq", 1100, 0.18, 0.1, duty=0.25, f1=700))
        elif name == "buzz":
            n = int(1.2 * SR)
            tt = np.arange(n) / SR
            f = 220 * (1 + 0.04 * np.sin(2 * np.pi * 28 * tt))
            ph = np.cumsum(f) / SR % 1.0
            a(t, np.where(ph < 0.3, 1.0, -1.0) * 0.06)
        elif name == "snore":
            a(t, noise(0.6, 0.08, None, hold=12))
            a(t + 0.7, tone("sq", 700, 0.3, 0.04, duty=0.5, f1=1100))
        elif name == "roar":
            a(t, noise(0.9, 0.3, 0.4, hold=8))
            a(t, tone("sq", 90, 0.9, 0.12, duty=0.5, f1=60))
        elif name == "crash":
            a(t, noise(0.7, 0.4, 0.25, hold=2))
        elif name == "gulp":
            a(t, tone("sq", 200, 0.1, 0.12, duty=0.5, f1=100))
            a(t + 0.16, tone("sq", 160, 0.12, 0.12, duty=0.5, f1=80))
        elif name == "whoosh":
            a(t, noise(0.35, 0.15, None, hold=6) * np.hanning(int(0.35 * SR)))
        elif name == "boo":
